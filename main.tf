...
resource "aws_db_parameter_group" "paramss" {
name = "paramss"
family = "mysql8.0"
...
resource "aws_db_instance" "aws_rds" {
  ...
  password             = "password"
  parameter_group_name = "${aws_db_parameter_group.paramss.name}"
  multi_az             = "false" 
  ...
  policy = <<EOF
{
    ...
    "Statement": [
        {"Action": [
			...
			"Effect": "Allow",
            "Resource": ["${aws_s3_bucket.lambda_codedeploy.arn}",
...
resource "aws_launch_configuration" "asg_launch_config" {
  ...
  user_data = <<-EOF
                ...
                sudo echo bucket=${aws_s3_bucket.webapp_mahesh_prasad.bucket} >> data.txt
                sudo echo S3_BUCKET_NAME=${aws_s3_bucket.webapp_mahesh_prasad.bucket} >> data.txt
                sudo echo TOPIC_ARN=${aws_sns_topic.password_reset.arn} >> data.txt
                sudo echo AWSAccessKeyId=${var.AWSAccessKeyId} >> data.txt
                sudo echo AWSSecretKey=${var.AWSSecretKey} >> data.txt
  EOF
  security_groups = ["${aws_security_group.application.id}"]
...
resource "aws_lambda_function" "my_awsLambdaFunc" {
...
protocol        = "lambda"
endpoint        = "${aws_lambda_function.my_awsLambdaFunc.arn}"
}

resource "aws_lambda_permission" "my_aws_lambda_permission" {
statement_id  = "AllowExecutionFromSNS"
action        = "lambda:InvokeFunction"
principal     = "sns.amazonaws.com"
source_arn    = "${aws_sns_topic.password_reset.arn}"
function_name = "${aws_lambda_function.my_awsLambdaFunc.function_name}"
}

resource "aws_iam_policy" "my_topicPolicy" {
name        = "Topic"
description = "topic_password_reset"
policy      = <<EOF
{
          "Version" : "2012-10-17",
          "Statement": [
...
resource "aws_iam_policy" "my_lambdaPolicy" {
name        = "lambda"
policy =  <<EOF
{
          "Version" : "2012-10-17",
...
resource "aws_iam_role_policy_attachment" "my_lambdaPolicy_attach_role" {
role       = "${aws_iam_role.CodeDeployLambdaServiceRole.name}"
policy_arn = "${aws_iam_policy.my_lambdaPolicy.arn}"
}

resource "aws_iam_role_policy_attachment" "my_topicPolicy_attach_role" {
role       = "${aws_iam_role.CodeDeployLambdaServiceRole.name}"
policy_arn = "${aws_iam_policy.my_topicPolicy.arn}"
}



resource "aws_iam_policy" "circle_ci_update_lambda_to_S3" {
name        = "circle_ci_update_lambda_to_S3"
path        = "/"
description = "Allows CircleCI to upload artifacts from latest successful build to dedicated S3 bucket used by code deploy"
policy = <<EOF
{
...
resource "aws_lb_listener" "lb_listener3" {
  ...
  port = 443
  protocol = "HTTPS"
  certificate_arn = "arn:aws:acm:us-east-1:734549157104:certificate/076dbe42-0474-4fad-b5dd-0d812010d2a8"
  default_action {
    type = "forward"
    target_group_arn = aws_lb_target_group.target-group-autoscale1.arn
    }
...
resource "aws_iam_role_policy_attachment" "my_lambdaPolicy_BasicExecRole" {
role       = "${aws_iam_role.CodeDeployLambdaServiceRole.name}"
...
resource "aws_lb_listener" "lb_listener4" {
  load_balancer_arn = aws_lb.my-load-Balancer.arn
  port = 8080
  protocol = "HTTPS"
  certificate_arn = "arn:aws:acm:us-east-1:734549157104:certificate/076dbe42-0474-4fad-b5dd-0d812010d2a8"
